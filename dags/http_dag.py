...
def _extract_data(**context):
    response = context['ti'].xcom_pull(task_ids='get_op') # 다른 서비스와 communicate를 위한 function
...
def _store_character(**context):
    ...
    hook.copy_expert(
        ...
        sql="COPY starwars_character FROM stdin WITH DELIMITER as ','" # CSV 타입으로 되어있으니 DELIMITER를 ,로 사용하여 데이터 베이스로 저장한다.
    )
